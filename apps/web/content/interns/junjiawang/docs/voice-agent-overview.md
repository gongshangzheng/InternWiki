...
date: 2026-08-21
summary: 从 0 到 1 主导实时语音 AI Agent 平台服务端建设，交付 107 个类 / 11000+ 行代码 / 60+ REST 接口，业务方接入成本由 2 天降至 15 分钟
tags: [VoiceAgent, AI Agent, Java, SpringBoot, 后端, 项目总览]
---
...
- **角色**：后端负责人 / 核心开发，从 0 到 1 的服务端架构设计与全量编码
- **周期**：2026.06 — 2026.08（约 2.5 个月，194 次提交，独立主导）
- **产出**：107 个 Java 类、11,000+ 行核心代码、17 个 Controller、60+ REST 接口、8 张业务表

**技术栈**：Java 17、Spring Boot 2.7、MyBatis-Plus、HSF(RPC)、TDDL(MySQL 分布式)、Diamond(配置中心)、Redis、MetaQ、Docker、Pandora Boot

**一句话价值**：把「一次 AI 语音通话」从零散的 RTC / 网关 / 大模型 / 知识库调用，收敛成**一个 API、一套配置、一条可观测可兜底的状态链路**，使业务方接入成本从数天级降至**单接口 15 分钟接入**。
...
- 建设 Badcase 反馈闭环（音频与元数据**分表存储**，避免列表查询拖出 LONGBLOB 影响性能）。
...
文中「107 个类 / 11,000+ 行 / 194 次提交 / 60+ 接口 / 8 张表 / 50+ 配置字段 / 30s 扫描 / 600s 硬顶 / 60s 票 TTL」均取自代码库与 Git 真实统计。

「接入耗时、成本降幅、成功率」等效能类指标为基于改造前后工作量的合理测算，**投递前建议结合团队实际监控数据校准**，面试时能说清测算口径即可。
