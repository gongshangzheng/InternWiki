...
同一机制把 `_lip` 换成 `_eye` 就能单独处理眼部。我们团队的 mouth-isolation 改动正是利用它：把唇通道的基底换成源首帧，避免源视频原有的嘴动泄露进来（详见 [Ditto 改动实践](Ditto_改动实践.md)）。
...
- **Avatar Forcing 微调**：FLOAT latent 的 identity-motion 分解（z = z_S + m_S）被我们用来做漂移治理的锚点——见 [Avatar Forcing 微调实践](Avatar_Forcing_微调实践.md)
