...
> 本文只讨论 Ditto 的实时化与 TensorRT 工程链，不讨论视频源嘴动泄露与闭嘴化方案，后者见 [Ditto 改动实践](Ditto_改动实践.md)。模型原理见 [Ditto 模型精读](Ditto_模型精读.md)，发布链与浏览器侧节奏见 [CyberVerse 工程专题](CyberVerse_工程专题.md)。项目全景见 [[project:digital-human]]。
...
其中发布侧同样关键：编码器 tail grace 修复后 cadence 从 `1.056–1.070` 降到 `1.018–1.020`，绝对播放锚点进一步使 240 秒会话稳态 cadence 达到 **1.000**。详见 [CyberVerse 工程专题](CyberVerse_工程专题.md)。
