...
> 本文固定阅读本地缓存的上游版本 `6712f62`。本地 adapter、FaceCropper 和性能冒烟见 [Talker-T2AV 接入与验证](Talker-T2AV_接入与验证.md)。音画同步模块边界见 [音画同步专题](音画同步专题.md)。项目全景见 [[project:digital-human]]。
...
因此后续的 LIA-X decoder 蒸馏目标不是“换个现代网络就画得更好”，而是用一个轻量学生 renderer 尽量逼近现有 LIA-X teacher，同时降低延迟和显存。这个方向目前是设计候选，尚未实现与验收；详见 [视频生成训练与推理加速专题](视频生成训练与推理加速专题.md)。
